...
<Shell>
    <svelte:fragment slot="area">
        ...
        <button class="chip variant-soft-secondary w-1/3 uppercase flex-col justify-center items-center border-double border-4 border-primary-500 py-1 text-black" >
            In progress
        ...
        <div class="card w-full h-full p-2 bg-white/40 " tabindex="-1">
            {#if modeShowTodo}
                ...
                <input bind:value={inputChip} placeholder="Search..."
                       class="input absolute right-0 bottom-0 w-full rounded-lg variant-glass-primary " type="search" name="todomode"
                />
            {:else}
                <Autocomplete bind:input={inputChip} denylist={inputChipList} options={flavorOptions} on:selection={onInputChipSelect}
