...
export const load: PageLoad = async ({ fetch: _fetch, params, parent, depends, route }) => {
    ...
    const artistRes = await fetch(`/spotify/api/spotify/artists/${params.id}`);
    if (!artistRes.ok) {
    ...
    if (artistResJSON?.images?.length > 0) {
        colorReq = fetch(
            `/spotify/api/average-color?${new URLSearchParams({
    ...
    const [albumsRes, appearsOnRes, topTracksRes, relatedArtistsRes, colorRes] = await Promise.all([
        fetch(`/spotify/api/spotify/artists/${params.id}/albums?limit=6&include_groups=album,single`),
        fetch(`/spotify/api/spotify/artists/${params.id}/albums?limit=6&include_groups=appears_on`),
        fetch(`/spotify/api/spotify/artists/${params.id}/top-tracks?market=${user?.country}`),
        fetch(`/spotify/api/spotify/artists/${params.id}/related-artists`),
        colorReq
    ]);
