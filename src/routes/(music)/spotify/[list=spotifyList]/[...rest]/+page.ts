...
export const load: PageLoad = async ({ params, fetch: _fetch, url, depends, route }) => {
    ...
    if (list === 'section' && rest === 'new-releases') {
        request = fetch(`/spotify/api/spotify/browse/new-releases?${searchParams}`);
        title = 'New Releases';
    } else if (list === 'section' && rest === 'featured-playlists') {
        request = fetch(`/spotify/api/spotify/browse/featured-playlists?${searchParams}`);
        title = 'Featured Playlists';
    } else if (list === 'category') {
        request = fetch(`/spotify/api/spotify/browse/categories/${rest}/playlists?${searchParams}`);
        const catInfo = await fetch(`/spotify/api/spotify/browse/categories/${rest}`);
        ...
        request = await fetch(`/spotify/api/spotify/me/following?type=artist&${searchParams}`);
        title = 'Following';
    } else if (list === 'artist') {
        ...
        const artistInfo = await fetch(`/spotify/api/spotify/artists/${artistID}`);
        ...
        if (dataType === 'albums') {
            request = fetch(
                `/spotify/api/spotify/artists/${artistID}/albums?include_groups=album,single&${searchParams}`
            );
            title = artistInfoJSON ? `${artistInfoJSON.name} Albums` : 'Albums';
        }
        if (dataType === 'appears-on') {
            request = fetch(
                `/spotify/api/spotify/artists/${artistID}/albums?include_groups=appears_on&${searchParams}`
            );
            title = artistInfoJSON ? `${artistInfoJSON.name} Appearances` : 'Appearances';
        }
        if (dataType === 'related-artists') {
            request = fetch(`/spotify/api/spotify/artists/${artistID}/related-artists`);
