...
<div class="flex-col h-full relative space-y-1">
	<div class="text-[19px] w-full h-1/2 font-digital font-bold">
		Current
		<span
			class="text-[16px] absolute right-0 top-0.5 border-b-4 border-emerald-700 text-emerald-600"
			>{$pomoInfo.cycle.count}
		</span>
	</div>
	<div class="text-[19px] w-full h-1/2 font-digital font-bold text-emerald-500">
		Goal
...
<div class="flex-col h-full space-y-1">
	...
	<div class="text-[18px] w-full h-1/2 font-digital font-bold text-center">
		{#if $pomoInfo.isRunning}
			{$pomoInfo.cycle.start.split(' ')[0]} - {$pomoInfo.cycle.end.split(' ')[0]}
		{:else}
	...
	<div class="text-[18px] w-full h-1/2 font-digital font-bold text-[#6a151c] text-center">
		{$pomoInfo.startTime.split(' ')[0]} - {$pomoInfo.endTime.split(' ')[0]}
	</div>
</div>
