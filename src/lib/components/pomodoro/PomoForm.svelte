...
<form
	class="flex-col w-full h-full p-5"
	action="#"
	on:mouseleave={() => {
		if (!set) timerSet = { ...timerDefaultSet };
	}}
>
		...
		<div class="flex-col w-full space-y-2">
			<div class="w-full flex space-x-3">
				<div class="flex w-1/2 justify-between space-x-2">
					...
					<input
						bind:checked={timerSet.alarmSession}
						type="checkbox"
						size="sm"
						class="w-5 h-5 input
			...
			<input
				class="input w-full h-full !text-surface-900 rounded-2xl variant-soft-surface mt-2"
				type="file"
				accept="audio/*"
	...
	<div class="flex w-full justify-between space-x-2 mt-5 border-t-2 pt-4 border-tertiary-500">
		<button
			class="w-1/4 rounded-full variant-glass-primary btn !text-primary-900"
			on:click={() => {
				timerSet = { ...timerDefaultSet };
		...
		<button
			class="w-3/4 shadow py-2 variant-soft-tertiary dark:text-tertiary-900 text-lg font-bold leading-5"
			on:click={() => {
				$settings = { ...timerSet };
